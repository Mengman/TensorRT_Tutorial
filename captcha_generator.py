...
class CaptchaGen(object):
    ...
    def image(self, captcha_str):
        ...
        img = cv2.copyMakeBorder(cv2.resize(img, (80, 30)), 1, 1, 0, 0, cv2.BORDER_CONSTANT, value=(255, 255, 255))
        img = img.transpose(1, 0).reshape((1, self.h, self.w))
...
class DigitCaptcha(object):
    ...
    def get_rand(num_digit_min, num_digit_max):
        ...
        for i in range(max_len):
            buf += chr(ord('a') + random.randint(0, 25))
        return buf
...
if __name__ == '__main__':
    ...
    def main():
        ...
        parser.add_argument("--char", help="Up to 4 letters [a-z; default: random]")
        args = parser.parse_args()

        captcha = ImageCaptcha(fonts=[args.font_path])
        captcha_str = args.char if args.char else DigitCaptcha.get_rand(3, 4)
        img = captcha.generate(captcha_str)
        img = np.fromstring(img.getvalue(), dtype='uint8')
        img = cv2.imdecode(img, cv2.IMREAD_GRAYSCALE)
        cv2.imwrite(args.output, img)
        print("Captcha image with digits {} written to {}".format([c for c in captcha_str], args.output))

    def main0():
        gen = CaptchaGen(80, 32, ['Ubuntu-M.ttf'])
        gen.image('abc')
    main()
